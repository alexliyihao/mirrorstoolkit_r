---
title: "Mirrorstoolkit: one-liner analysis tool"
author:
- name: "Yihao Li"
  affiliation: 
  - "Reyes-Soffer Lab, Division of Preventive medicine, Department of Medicine, Columbia University Irving Medical Center; "
  - "Badri Vardarajan Lab, Gertrude H. Sergievsky Center, Department of Neurology, Columbia University Irving Medical Center"
output: BiocStyle::html_document
vignette: >
  %\VignetteIndexEntry{Mirrorstoolkit: A quick one-liner analysis tool in R}
...
lipidomics = readxl::read_xlsx("/Users/alexli/Library/CloudStorage/OneDrive-ColumbiaUniversityIrvingMedicalCenter/Lpa_metabolism_Gissette/cleaned_data/20241011_WHICAP_lipidomics/lipidomics_mol_percentage_subcategorized.xlsx")
serum_info = readxl::read_excel("/Users/alexli/Library/CloudStorage/OneDrive-ColumbiaUniversityIrvingMedicalCenter/Lpa_metabolism_Gissette/cleaned_data/WHICAP_phenotypes/masterlist_2024_sample.xlsx")
lipidomic_samples = base::merge(  
  x = serum_info %>% dplyr::select(tidyr::all_of(c("WES_ID", "Lp(a) (nmol/L)"))),
  y = lipidomics,
  by = "WES_ID") %>%
  dplyr::select(
      c("WES_ID", "Lp(a) (nmol/L)", dplyr::starts_with("DG"))
    )
```
...
With the result above, you can make your decision and generate a characteristic table. 
...
* For variables in boolean\_list, the p-value is from chi-square test (stats::chisq.test())
* For variables in IQR\_list, the p-value is from Kruskal–Wallis test (stats::kruskal.test())
* For varaibles in sd\_list, the p-value is depending on variable *normal_policy*
...
mirrorstoolkit::correlation_matrix_plot will return a ggplot2 instance, sharing a similar argument setting:

 - variable is optional, if not provided, will use all the columns provided in data. However, as a fool-proofing operation, providing the variable column is a good sanity check as the function will internally enforces all the value in your data table to numeric.
 - font_size is 5 by default for all labels.
 - please be aware that the order is reordered by hierarchical clustering for better cluster illustration
...
This function also provide a filter operation by the p-value as long as the p_threshold is specified. see below (I changed the way inputting column here for illustration purpose, it doesn't matter):
...
For potential multi-omics studies in the future, I left an function here outputting the matrix as data.frame format, which removed all the redundant columns. It has a p_threshold option same as above but I don't think it's necessary here.
...
You can either specify the column you want by variable = c("col1", "col2", "col3") if you need, but most of the times it will be quite lengthy. If it is not specified, it will use all the columns not mentioned in the table.
...
OLS_wrapper and logistic_wrapper() can deal with most of the association analysis we need. In most of our tasks, we can specify the follow 2 or 3 lists as global settings used repetitively.
...
- For each column name mentioned in *variable_list*, let's call it *variable_i*
  * Grab the columns mentioned in *variable_i*, *adjustments*, and *response* from *data*. If *variable_i* is in *adjustments*, will remove the duplicates
  * Logistic regression *response* $\sim$ *variable_i* + *adjustment* + 1
  * Seek the effect size and coefficient of *variable_i*, compute the odd ratio by exp(effect size)
...
I called it OLS because machine learning community call it so... Forgive me. Ordinary linear regression doesn't have OR defined thus only beta and p-value here.
...
```{r, include = FALSE}
# Quickly initiate a dataset
n <- 30000
base::set.seed(42)
df = cbind(
    factor1 = rnorm(n, 1, 0.05),
    factor2 = rnorm(n, 0, 1),
    age = rnorm(n, 56, 5),
    bmi = rnorm(n, 26, 5),
    sex = sample(c(0,1), n,TRUE)
    ) %>% as.data.frame()
L = -5*log(df$factor1)+5*exp(df$factor2) + 0.05*df$bmi + 0.5*df$sex + 0.25*df$age
L = (L-L %>% min())/(L %>% max() - L %>% min())
y <- base::ifelse(stats::runif(n) < stats::plogis(L), 1, 0)
```
```{r}
head(df)
head(y)
