...
* *p_threshold* will affect the filtering from first to second table, the significant result will be labelled on the top of the figure
...
```{r}
pairwise_wilcox = pairwise_wilcox_report(
  data = df, 
  variable = "Albumin", 
  by = "group"
)
...
```
If you only want the first table in supplemental data, you can run the one below as well. Replace "report" to "table" in the function name and remove the p_threshold if you have. This function is internally called in mirrorstoolkit::pairwise_t_test_report, so does the other two.
```{r}
